...
{
	uint indexCounter;   // counts the size of nonemptyList
	uint vertexCounter;  // how many vertices do we need to allocate storage for for next compute stage's output
	uint nonemptyList[];
};
...
void main(){
	...
	*/
	...
	if(cubecase > 0 && cubecase < 255){
		...
		atomicAdd(vertexCounter, numverts);
		uint index = atomicAdd(indexCounter, 1);
