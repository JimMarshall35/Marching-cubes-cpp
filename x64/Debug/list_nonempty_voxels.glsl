...
void main(){
	...
	*/
	...
	float bottom_back_left   = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(0,0,0))).x;
	float bottom_front_left  = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(0,0,1))).x;
	float top_back_left      = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(0,1,0))).x;
	float top_front_left     = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(0,1,1))).x;
	float bottom_back_right  = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(1,0,0))).x;
	float bottom_front_right = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(1,0,1))).x;
	float top_back_right     = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(1,1,0))).x;
	float top_front_right    = imageLoad(densityTexture,ivec3(gl_WorkGroupID*4 + gl_LocalInvocationID + uvec3(1,1,1))).x;
	...
	if(cubecase > 0 && cubecase < 255){
		...
		uint z8_y8_x8_case8 = ((gl_WorkGroupID.z*4 + gl_LocalInvocationID.z) << 24) | ((gl_WorkGroupID.y*4 + gl_LocalInvocationID.y) << 16) | ((gl_WorkGroupID.x*4+ gl_LocalInvocationID.x) << 8) | cubecase;
